...
class phpBB3_HookLoaderMain_Adm_Panel extends ObjectFileSystemFile
{
	public $lang;
	...
	function construct()
	{
		$this->Info	= new phpBB3_HookLoaderMain_Adm_PanelInfo();
		$this->Var 	= new phpBB3_HookLoaderMain_Adm_PanelVar();
	}
		
	function main()
	{
		$this->Var->set_request_var('tabmenu', 'HookLoader');
		$this->plugin_name = $this->Var->get_request_var('tabmenu');
		$Plugin = new phpBB3_HookLoaderPlugin($this->plugin_name);
		$this->PluginAdm = $Plugin->Adm();
		$haveIntro = $this->PluginAdm->haveIntro();
		$this->Var->set_request_var('sidemenu', ($haveIntro)? 'intro' : 'pluginInfo');
		...
		$this->create_errorblock();
	}

	private function create_errorblock()
	{
		global $template;

		if ($this->plugin_name === 'HookLoader')	{
			return;
		}
...
class phpBB3_HookLoaderMain_Adm_PanelInfo
{
	private $adm;
...
class phpBB3_HookLoaderMain_Adm_PanelVar
{
