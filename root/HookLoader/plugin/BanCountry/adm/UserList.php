...
class phpBB3_HookLoaderPluginAdm_BanCountry_UserList {
	...
	private function draw_userlist() {
		...
		if (! in_array ( $user_country, array (
				'localnetwork',
				'unknown' 
		) )) {
		...
		foreach ( $userlist as $i => $row ) {
			...
			if ($countryListType !== '') {
				if ($countryListType === self::countryListType_BANNED) {
					if ($this->API->Plugin->config ['countryListType'] && in_array ( $row ['user_country'], $countryList ) || ! $this->API->Plugin->config ['countryListType'] && ! in_array ( $row ['user_country'], $countryList )) {
						continue;
					}
